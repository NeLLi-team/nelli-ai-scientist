...
class UniversalMCPAgent:
    ...
    async def initialize(self):
        ...
        if self.mcp_servers:
            ...
            logger.info(
                f"✅ Discovered {len(self.discovered_tools)} tools from {len(self.mcp_servers)} servers"
            )
        else:
        ...
        try:
            ...
            for server_id, server_config in mcp_config.get("mcp_servers", {}).items():
                ...
                if enabled:
                    ...
                    logger.info(
                        f"    ✓ Loaded MCP server: {server_config.get('name', server_id)}"
                    )
    ...
    async def _discover_server_tools(
        ...
        try:
            ...
            async with Client(connection_params) as client:
                ...
                logger.info(
                    f"  ✅ Loaded {len(tools)} tools: {', '.join(server_tools.keys())}"
                )

        except ImportError:
            logger.warning(
                f"FastMCP client not available. Cannot discover tools from {server_id}"
            )
        except Exception as e:
            logger.error(
                f"Failed to discover tools from {server_id}: {type(e).__name__}: {e}"
            )
            import traceback
    ...
    async def _call_mcp_tool(self, tool_name: str, **kwargs) -> Dict[str, Any]:
        ...
        if tool_name not in self.discovered_tools:
            return {
                "error": f"Tool '{tool_name}' not found. Available tools: {list(self.discovered_tools.keys())}"
            }
        ...
        logger.info(
            f"🔄 Calling tool '{tool_name}' on server '{tool_info['server_name']}'"
        )
    ...
    async def process_natural_language(self, user_input: str) -> Dict[str, Any]:
        ...
        prompt = self.prompt_manager.format_prompt(
            "tool_selection", tools_context=tools_context, user_input=user_input
        )
        ...
        for category_info in self.tool_categories.values():
            ...
            for tool_name in category_info["tools"]:
                ...
                if all_params:
                    params_str = ", ".join(
                        [
                            f"{p}{'*' if p in required_params else ''}"
                            for p in all_params
                        ]
                    )
    ...
    async def execute_tool_suggestion(
        self, suggestion: Dict[str, Any], previous_results: List[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    def _resolve_chained_parameters(
        self, parameters: Dict[str, Any], previous_results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                # Execute suggested tools
                    ...
                    for i, suggestion in enumerate(analysis["suggested_tools"]):
                        ...
                        result = await self.execute_tool_suggestion(
                            suggestion, tool_results
                        )
                    ...
                    if tool_results:
                        ...
                        reflection = await self._reflect_on_tool_results(
    ...
    def _display_welcome(self):
        ...
        if self.discovered_tools:
            ...
            for category in self.tool_categories.values():
                print(
                    f"  \033[36m•\033[0m {category['name']}: \033[33m{len(category['tools'])}\033[0m tools"
                )
        else:
        ...
        print(
            "\033[35m💬 Or just type naturally to interact with available tools\033[0m"
        )
    ...
    async def _generate_response(self, user_input: str) -> str:
        ...
        prompt = self.prompt_manager.format_prompt(
            "general_response", user_input=user_input
        )
    ...
    async def _reflect_on_tool_results(
        ...
        for result in tool_results:
            ...
            if isinstance(tool_result, dict) and "error" not in tool_result:
                results_context.append(
                    f"Tool '{tool_name}' executed successfully with result: {json.dumps(tool_result, indent=2)}"
                )
            elif isinstance(tool_result, dict) and "error" in tool_result:
                results_context.append(
                    f"Tool '{tool_name}' encountered an error: {tool_result['error']}"
                )
            else:
                results_context.append(
        ...
        prompt = self.prompt_manager.format_prompt(
            "reflection", user_request=user_request, results_summary=results_summary
        )
