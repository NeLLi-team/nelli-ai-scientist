...
# Import the stdio-compatible agent for base functionality  
from .agent_stdio import UniversalMCPAgentStdio as UniversalMCPAgent, AgentConfig
...
class EnhancedUniversalMCPAgent(UniversalMCPAgent):
    ...
    def __init__(self, config: EnhancedAgentConfig, config_loader: ConfigLoader = None):
        ...
        # Create separate LLM interface for reasoning and planning with more capable model
        self.reasoning_and_planning_llm = LLMInterface(provider=config.llm_provider)
        # Override the model for reasoning and planning
        if hasattr(self.reasoning_and_planning_llm.llm, 'model'):
            self.reasoning_and_planning_llm.llm.model = self.config_loader.get_reasoning_and_planning_model()
        
        # Create separate LLM interface for execution with faster model
        self.execution_llm = LLMInterface(provider=config.llm_provider)
        # Override the model for execution
        if hasattr(self.execution_llm.llm, 'model'):
            self.execution_llm.llm.model = self.config_loader.get_execution_model()
        
        # Initialize planning components (use reasoning/planning LLM)
        self.task_planner = TaskPlanner(self.reasoning_and_planning_llm, self.prompt_manager)
    ...
    async def process_natural_language_with_planning(self, user_input: str) -> Dict[str, Any]:
        ...
        if self.config_loader.is_reasoning_enabled():
            ...
            # Fallback reasoning - check if this looks like an analytical request
            analytical_keywords = [
                'stats', 'statistics', 'analysis', 'analyze', 'calculate', 'compute', 
                'assembly', 'genome', 'sequence', 'gene', 'average', 'distribution',
                'quality', 'metrics', 'length', 'count', 'percentage', 'ratio'
            ]
            
            # If it's analytical, mark as COMPLEX to ensure planning
            complexity = TaskComplexity.COMPLEX if any(keyword in user_input.lower() for keyword in analytical_keywords) else TaskComplexity.MODERATE
            ...
            reasoning_result = ReasoningResult(
                ...
                approach_strategy="Adaptive execution with planning if needed"
            )
        ...
        if (self.config_loader.is_planning_enabled() and 
            ...
            # Simple direct execution for basic tasks using execution LLM
            print("\n🚀 \033[36mDirect Execution (Simple Task)...\033[0m")
            # Temporarily swap to execution LLM for simple tasks
            original_llm = self.llm
            self.llm = self.execution_llm
            try:
                result = await self.process_natural_language(user_input)
                
                # Check if execution failed and this seems like an analytical request
                if (result is None or result.get("needs_clarification") or 
                    "trouble processing" in str(result).lower()):
                    
                    # Check if this is an analytical request that should have gone to planning
                    analytical_keywords = [
                        'stats', 'statistics', 'analysis', 'analyze', 'calculate', 'compute', 
                        'assembly', 'genome', 'sequence', 'gene', 'average', 'distribution',
                        'quality', 'metrics', 'length', 'count', 'percentage', 'ratio'
                    ...
                    if any(keyword in user_input.lower() for keyword in analytical_keywords):
                        print("\n🔄 \033[33mDirect execution failed for analytical request - switching to planning mode...\033[0m")
                        
                        # Force complexity to COMPLEX and trigger planning
                        reasoning_result.complexity_assessment = TaskComplexity.COMPLEX
    ...
    async def _execute_planned_workflow(self) -> Dict[str, Any]:
        ...
        while True:
            ...
            if execution_error or tool_error:
                ...
                print(f"❌ \033[31mStep encountered an issue:\033[0m {error_msg}")
                ...
                if step.retry_count <= step.max_retries:
                    ...
                    # Get recovery action from error handler
                    if hasattr(self, 'error_handler'):
                        recovery = self.error_handler.suggest_recovery_action(
                            self.error_handler.classify_error(Exception(error_msg), error_context),
                            error_context
                        )
                        print(f"⚠️  \033[33m{recovery['message']}\033[0m")
                    
                    plan.update_step_status(step.id, StepStatus.FAILED, error=error_msg)
                    if self.config_loader.is_progress_tracking_enabled():
                        self.progress_tracker.update_step_progress(
                            self.current_plan_id, step.id, StepStatus.FAILED, error=error_msg
                        )
            else:
    ...
    async def _resolve_step_parameters(self, step: ExecutionStep) -> Dict[str, Any]:
        ...
        for param_name, param_value in step.parameters.items():
            if param_value == "ANALYSIS_RESULTS":
                if self.execution_results:
                    ...
                    resolved_params[param_name] = {}
            elif param_value == "CODE_FROM_PREVIOUS_STEP":
                if self.execution_results:
                    # Look for the most recent code generation result
                    for result in reversed(self.execution_results):
                        if not result.get("error") and result.get("result"):
                            tool_result = result["result"]
                            # Look for generated code in the result
                            if isinstance(tool_result, dict) and "code" in tool_result:
                                resolved_params[param_name] = tool_result["code"]
                                break
                            elif isinstance(tool_result, dict) and "result" in tool_result and isinstance(tool_result["result"], dict) and "code" in tool_result["result"]:
                                resolved_params[param_name] = tool_result["result"]["code"]
                                break
                    else:
                        # Fallback if no code found
                        resolved_params[param_name] = "print('No code was generated from previous step')"
                else:
                    resolved_params[param_name] = "print('No previous steps executed')"
            elif param_value == "PREVIOUS_TOOL_RESULT":
                if self.execution_results:
                    # Use the most recent successful result
                    for result in reversed(self.execution_results):
                        if not result.get("error") and result.get("result"):
                            resolved_params[param_name] = result["result"]
                            break
                    else:
                        resolved_params[param_name] = {}
                else:
        ...
        # Use ParameterResolver for additional validation
        if hasattr(self, 'error_handler'):
            # Special handling for path parameters
            if param_name in ["path", "file_path", "directory"]:
                resolved_params[param_name] = ParameterResolver.resolve_path_parameter(
                    resolved_params.get(param_name)
                )
            # Special handling for pattern parameters
            elif param_name in ["pattern", "file_pattern", "search_pattern"]:
                resolved_params[param_name] = ParameterResolver.resolve_pattern_parameter(
                    resolved_params.get(param_name)
                )
            # Special handling for boolean parameters
            elif param_name in ["recursive", "ignore_hidden", "include_hidden"]:
                resolved_params[param_name] = ParameterResolver.resolve_boolean_parameter(
                    resolved_params.get(param_name)
                )
...
"""
    ...
    def _display_enhanced_welcome(self):
        ...
        print(f"\033[32mReasoning & Planning Model:\033[0m {self.config_loader.get_reasoning_and_planning_model()}")
        print(f"\033[32mExecution Model:\033[0m {self.config_loader.get_execution_model()}")
